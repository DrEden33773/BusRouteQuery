...
requires StdOut<T>
class Graph {
    friend class Utility::Dijkstra<T>;
    friend class Utility::Floyd<T>;
...
public:
    ...
    void DFS_Rec_Func(HashSet& visited_idx, const int& idx) {
        ...
        std::vector<int>&& adj_idx_list
            = FindAllAdjIndex(Index_V_Map.at(idx));
    ...
    void DFS(const T& vex, HashSet& visited_idx) {
        if (!if_has_vex(vex)) {
            throw std::logic_error("Input vertex is NOT exist!");
        }
        if (!size) {
    ...
    void BFS(const T& vex, HashSet& visited_idx) {
        if (!if_has_vex(vex)) {
            throw std::logic_error("Input vertex is NOT exist!");
        }
        if (!size) {
